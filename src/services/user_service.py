...
class UserService:
    ...
    def create_or_update_user(self, user_id, avatar_url=None):
        logging.info(f"UserService: Attempting to create or update user {user_id} with provided avatar_url: {avatar_url}")
        if user_id not in self.users:
            ...
            self.users[user_id] = User(user_id=user_id, avatar_url=avatar_url)
            logging.info(f"UserService: Created new user {user_id} with avatar: {avatar_url}")
        else:
